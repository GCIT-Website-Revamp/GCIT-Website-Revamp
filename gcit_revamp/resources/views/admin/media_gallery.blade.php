                           ...
                           <div class="d-flex justify-content-between align-items-center">
                              ...
                              <div>
                                 <button id="addClubBtn" type="button" class="btn btn-success" data-bs-toggle="modal"
                                    ...
                                    <i class="fa fa-plus-circle mx-2 mb-2 mt-2"></i> Add Media
                                 </button>
                              </div>
                           </div>
                        </div>
                              ...
                              <table class="table">
                                 ...
                                 <tbody>
                                    @forelse ($media as $index => $item)
                                       <tr>
                                          <td style="max-width: 60px;">{{ $item->position }}</td>
                                          <td>{{ $item->title }}</td>
                                          <td>
                                             @php
                                                   $path = asset('storage/' . $item->media);
                                                   $extension = strtolower(pathinfo($item->media, PATHINFO_EXTENSION));
                                                   $isVideo = in_array($extension, ['mp4', 'avi', 'mov', 'mkv', 'webm']);
                                             @endphp
                                             @if ($isVideo)
                                                   <video width="120" controls style="border-radius: 5px;">
                                          ...
                                          <td style="max-width: 60px;">
                                             <div class="action-buttons">
                                                   <button type="button" class="btn btn-success edit-media-btn"
                                                      data-item-id="{{ $item->id }}" 
                                                      data-item-title="{{ $item->title }}"
                                                      data-item-position="{{ $item->position }}"
                                                      data-item-media="{{ asset('storage/' . $item->media) }}">
                                                      Edit
                                                   </button>
