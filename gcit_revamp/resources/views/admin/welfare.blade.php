...
<body class="inner_page tables_page">
                                ...
                                <div class="full graph_head">
                                    <div class="d-flex justify-content-between align-items-center">
                                        <div class="heading1 margin_0">
                                            <h2>Student Welfare Information</h2>
                                        </div>
                                        <div>
                                            @if ($welfare)
                                                <button id="editWelfareBtn" type="button" class="btn btn-success"
                                                    data-bs-toggle="modal" data-bs-target="#myModal"
                                                    data-welfare-id="{{ $welfare->id }}"
                                                    data-welfare-description="{{ $welfare->description }}">
                                                    <i class="fa fa-pencil mx-2 mb-2 mt-2"></i> Edit
                                                </button>
                                            @else
                                                <button id="addWelfareBtn" class="btn btn-success" data-bs-toggle="modal"
                                ...
                                <div class="table_section padding_infor_info">
                                    <div id="admissionView" style="color:black">
                                        @if ($welfare)
                                            {!! $welfare->description !!}
                                        @else
                                            <p>No student welfare information added yet.</p>
                                        @endif
                                    </div>
                                </div>
                            </div>
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", () => {
            ...
            document.getElementById("addWelfareBtn")?.addEventListener("click", function () {
                // Set modal title
                document.querySelector('#myModal .modal-title').textContent = 'Add Student Welfare Information';

                // Modal body (CKEditor text area)
                document.querySelector('#myModal .modal-body').innerHTML = `
            <form id="welfareForm">
                <div class="form-group">
                    <label for="ictEditor">Student Welfare Description</label>
                    <textarea class="form-control" id="welfareEditor" name="description" rows="15" required></textarea>
                </div>
            </form>
        `;
            ...
            <button type="button" class="btn btn-danger" data-dismiss="modal">Close</button>
            <button type="button" class="btn btn-success" id="saveWelfare">Save</button>
        `;
                ...
                // Init CKEditor
                ClassicEditor
                    .create(document.querySelector('#welfareEditor'))
                    .then(editor => window.ictEditorInstance = editor)
                    .catch(err => console.error(err));

                // Save Handler
                document.getElementById("saveWelfare").addEventListener("click", function () {
                    const payload = {
                        description: window.ictEditorInstance.getData()
                    };
                    Swal.fire({
                        title: "Add Student Welfare Information?",
                        icon: "question",
                        ...
                        fetch(`/api/welfare`, {
                            ...
                            .then(data => {
                                if (data.success) {
                                    Swal.fire({
                                        icon: "success",
                                        title: "Student Welfare Information Added",
                                        text: data.message || "Student Welfare Information Added successfully!"
                                    });
            ...
            // ===============================
            document.getElementById("editWelfareBtn")?.addEventListener("click", function () {
                const welfareId = this.dataset.welfareId;
                const description = this.dataset.welfareDescription;

                document.querySelector('#myModal .modal-title').textContent = 'Edit Student Welfare Information';

                document.querySelector('#myModal .modal-body').innerHTML = `
            <form id="welfareEditForm">
                <div class="form-group">
                    <label for="welfareEditor">Student Welfare Description</label>
                    <textarea class="form-control" id="welfareEditor" name="description" rows="15">${description}</textarea>
                </div>
            </form>
        `;
                ...
                document.querySelector('#myModal .modal-footer').innerHTML = `
            <button type="button" class="btn btn-danger" id="deleteWelfare">Delete</button>
            <button type="button" class="btn btn-success" id="updateWelfare">Update</button>
        `;
                ...
                // Update Handler
                // =======================
                document.getElementById("updateWelfare").addEventListener("click", () => {
                    const payload = {
                        description: window.ictEditorInstance.getData()
                    };

                    Swal.fire({
                        title: "Update Student Welfare Information?",
                        icon: "question",
                        showCancelButton: true,
                        ...
                        fetch(`/api/welfare/${welfareId}`, {
                            ...
                            .then(data => {
                                if (data.success) {
                                    Swal.fire({
                                        icon: "success",
                                        title: "Student Welfare Information Updated",
                                        text: data.message || "Student Welfare Information Updated successfully!"
                                    });
                                    setTimeout(() => location.reload(), 1500);
                                } else {
                                    Swal.fire({
                ...
                // Delete Handler
                // =======================
                document.getElementById("deleteWelfare").addEventListener("click", () => {
                    Swal.fire({
                        title: "Delete Student Welfare Information?",
                        ...
                        fetch(`/api/welfare/${welfareId}`, {
                            ...
                            .then(data => {
                                if (data.success) {
                                    Swal.fire({
                                        icon: "success",
                                        title: "Student Welfare Information Deleted",
                                        text: data.message || "Student Welfare Information Deleted successfully!"
                                    });
